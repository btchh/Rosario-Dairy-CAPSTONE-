...
class ProfilePhotoTests(TestCase):
    """
    Covers the new profile-photo upload/delete feature: POST replaces
    (and cleans up) any existing photo, DELETE clears it, bad content
    types and oversized files are rejected, and GetUserView surfaces
    the current photo URL (or None).
    """
    ...
    @staticmethod
    def _make_image_file(name='test.png', content_type='image/png', size=100):
        from django.core.files.uploadedfile import SimpleUploadedFile
        # minimal valid PNG header + padding so it's a "real" small file
        content = b'\x89PNG\r\n\x1a\n' + b'\x00' * size
        return SimpleUploadedFile(name, content, content_type=content_type)

    def test_upload_succeeds_and_sets_photo(self):
        photo = self._make_image_file()
        response = self.client.post('/accounts/profile-photo/', {'photo': photo}, format='multipart')
        self.assertEqual(response.status_code, 200)
        self.user.refresh_from_db()
        self.assertTrue(bool(self.user.profile_photo))

    def test_get_user_view_reflects_uploaded_photo(self):
        photo = self._make_image_file()
        self.client.post('/accounts/profile-photo/', {'photo': photo}, format='multipart')
        response = self.client.get('/accounts/user/')
        self.assertEqual(response.status_code, 200)
        self.assertIsNotNone(response.data['profile_photo'])

    def test_get_user_view_shows_null_when_no_photo(self):
        response = self.client.get('/accounts/user/')
        self.assertIsNone(response.data['profile_photo'])

    def test_missing_file_returns_400(self):
        response = self.client.post('/accounts/profile-photo/', {}, format='multipart')
        self.assertEqual(response.status_code, 400)

    def test_invalid_content_type_rejected(self):
        bad_file = self._make_image_file(name='not_an_image.txt', content_type='text/plain')
        response = self.client.post('/accounts/profile-photo/', {'photo': bad_file}, format='multipart')
        self.assertEqual(response.status_code, 400)

    def test_oversized_file_rejected(self):
        big_file = self._make_image_file(size=6 * 1024 * 1024)  # 6MB > 5MB limit
        response = self.client.post('/accounts/profile-photo/', {'photo': big_file}, format='multipart')
        self.assertEqual(response.status_code, 400)

    def test_uploading_new_photo_replaces_old_one(self):
        first = self._make_image_file(name='first.png')
        self.client.post('/accounts/profile-photo/', {'photo': first}, format='multipart')
        self.user.refresh_from_db()
        first_name = self.user.profile_photo.name

        second = self._make_image_file(name='second.png')
        self.client.post('/accounts/profile-photo/', {'photo': second}, format='multipart')
        self.user.refresh_from_db()
    ...
    def test_delete_clears_photo(self):
        photo = self._make_image_file()
        self.client.post('/accounts/profile-photo/', {'photo': photo}, format='multipart')
        response = self.client.delete('/accounts/profile-photo/')
        self.assertEqual(response.status_code, 200)
        self.user.refresh_from_db()
        self.assertFalse(bool(self.user.profile_photo))

    def test_delete_when_no_photo_does_not_error(self):
        response = self.client.delete('/accounts/profile-photo/')
        self.assertEqual(response.status_code, 200)
